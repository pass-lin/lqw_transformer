...
class MultiHead(keras.layers.Layer):
    ...
    def set_weights(self,input_shape):
...
class AFT_full(MultiHead):
    ...
    def set_weights(self,input_shape):
        self.bias=self.add_weight(name='bias', 
                                ...
                                trainable=True)
        super(AFT_full, self).set_weights(input_shape)
    def mask_martic(self,k):
        seq_len=K.shape(k)[1]
        idxs = K.arange(0, seq_len)
        mask = idxs[None, :] <= idxs[:, None]
...
class Synthesizer_R(AFT_full):
    ...
    #也可以用作MLP-Mixer
     def __init__(self,train_able=True,**kwargs):
         ...
         if 'n_head' in kwargs.values():
             self.n_head=kwargs['n_head']
         if self.n_head>1:
             self.out_dims=self.n_head * self.head_dim
         else:
             self.head_dim=self.out_dims
     def set_weights(self,input_shape):
        self.wv = keras.layers.Dense(self.n_head * self.head_dim,use_bias=self.use_bias,kernel_initializer=self. kernel_initializer)
        ...
        self.o_dense = keras.layers.Dense(self.n_head * self.head_dim,use_bias=self.use_bias,kernel_initializer=self. kernel_initializer)
        self.o_drop = keras.layers.Dropout(rate=self.drop_rate)
        super(Synthesizer_R, self).set_weights(input_shape)
     def call(self, x, mask):     
        if self.use_Time_shift_only:
            x=self.time_shift(x)
        if self.n_head>1:
        ...
        v=self.wv(x) 
        r=self.R[:K.shape(x)[1],:K.shape(x)[1]]
        context =keras.activations.softmax(r)@v     # [n, q_step, h*dv]
        if self.n_head>1:
